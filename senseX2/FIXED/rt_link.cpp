...
/* Shouldn't changes as we are just returning 
*  the signal
*/
...
/* This function is supposed to return the current slot
* that is running on RT LINK
*/
uint16_t rtl_get_global_slot ()
{
    ...
    nrk_time_get (&ct); // this will load the current time into ct

    //printf( "ct %lu %lu\n",ct.secs,ct.nano_secs );
    v = nrk_time_sub (&ct, ct, last_slot_time); // this will subtract current time from last slot time and put it in ct
...
*/
    ...
    if(global_slot>=MAX_SLOTS || global_slot<2)
    	...
    	last_sync_slot = global_slot-2; 
			// slot before real last sync slot
...
/**Chinmay:Added this to resolev some linking issues
 * _rtl_rx()
 *
...
return NRK_OK;
	...
	void rtl_nw_task ()
	{
		...
		// This call is required to clear abs schedules
			...
			//printf("slots until next %d \r \n", next_slot_offset); // you get 2 and 30
